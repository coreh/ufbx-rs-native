...
pub unsafe extern "C" fn ufbx_load_memory(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_memory(data, size, opts) } {
        Ok(result) => {
...
pub unsafe extern "C" fn ufbx_load_file(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_file(filename, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_load_file_len(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_file_len(filename, filename_len, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_load_stdio(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_stdio(file_void, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_load_stdio_prefix(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_stdio_prefix(file_void, prefix, prefix_size, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_load_stream(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_stream(stream, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_load_stream_prefix(
    ...
    // own raw-pointer contract. The native impl is `Result`-shaped; this shim
    // owns the C slot writes (PORTING.md "Trailing `ufbx_error *error`").
    match unsafe { crate::native::api::load_stream_prefix(stream, prefix, prefix_size, opts) } {
        Ok(result) => {
            if !result.is_null() && !error.is_null() {
                // SAFETY: `error` is non-null (checked) and the caller's live
                // slot per this shim's contract; C clears it on success (and
...
pub unsafe extern "C" fn ufbx_find_prop_concat(
    ...
    // SAFETY: an ABI shim; the source pointer is bridged to a read-only
    // `View<_, Const>` (sound for any readable provenance) and the caller's
    // `parts`/`num_parts` key-array contract becomes one `ConcatParts` vouch.
    match unsafe {
        let parts = crate::prelude::slice_from_ptr(parts, num_parts);
        crate::native::api::find_prop_concat(
        crate::native::view::View::<crate::generated::Props, crate::native::view::Const>::from_ptr(
            props,
        ),
        crate::native::string_pool::ConcatParts::from_strings(parts),
    )
    } {
        Some(prop) => prop.as_ptr() as *mut crate::generated::Prop,
        None => core::ptr::null_mut(),
