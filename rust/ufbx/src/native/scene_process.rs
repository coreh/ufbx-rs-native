...
use crate::native::read::{
    ...
    setup_scale_helper, sort_properties, strblob_data, strblob_length, strblob_set_blob,
    strblob_set_string, unscaled_transform_to_matrix, update_vertex_first_index, NodeExtra,
    Strblob, SENTINEL_INDEX_CONSECUTIVE, SENTINEL_INDEX_ZERO,
};
...
impl<T> crate::prelude::RefListView<T> {
    ...
    pub(crate) fn as_element_list(&self) -> &crate::prelude::RefListView<Element> {
        ...
        // `ufbx_element_list` — a `data` pointer plus a `count` — so the re-view
        // addresses exactly the two leaves this Mut view already covers and
        // inherits its write-capable provenance. The projection reads neither
        // leaf and may therefore carry an uninitialized out-parameter header;
        // before any getter/index operation, data/count must describe `Ref<T>`
        // slots naming `ufbx_element`-headed elements.
...
// `find_dst_connections`). The typed walk preserves C's optional
// instance/helper traversal without recasting an Element header to Node.
#[inline(never)]
fn fetch_dst_elements(
    uc: &Context,
    element_flags: ElementFlags<'_>,
    p_dst_list: &RefListView<Element>,
...
pub(crate) fn finalize_shader_texture<'a>(
    ...
    unsafe {
        ...
        for prop in props {
                ...
                || sp::remove_suffix_c(StringView::from_mut(&mut base_name), b"Enabled\0")
            {
                ...
                if let Some(base) = base {
                    base.set_texture_enabled_prop(Some(prop.to_ref()));
                    continue;
                }
            }
    ...
    if !uc.opts_view().disable_quirks() {
        ...
        // table whose name fields are NUL-terminated literals, `input` is
        // null-checked before its deref, and its `prop` link is set by the input
        // loop above, so the unchecked unwrap of that field holds.
        unsafe {
            ...
            for i in 0..FILE_SHADERS.len() {
                    ...
                    || strcmp((*shader).shader_name.data, (*fs).shader_name) == 0
                {
                    ...
                    if !input.is_null() {
                        ...
                        // C-parity: `prop` is dereferenced unconditionally
                        // (ufbx.c:20677-20679) — every input the loop above
                        // creates carries the prop it was made from.
                        let prop: &PropView = ptr::read(&raw const (*input).prop)
                            .unwrap_unchecked()
                            .view();
...
pub(crate) fn fetch_file_textures(uc: &Context) -> Result<(), Fail> {
    ...
    loop {
        ...
        let shader: *mut ShaderTexture =
            unsafe { ptr::read(&raw const (*texture).shader) }.map_or(ptr::null_mut(), |r| r.ptr());
            ...
            unsafe {
                *states.add((*texture).element.typed_id as usize) =
                ...
                while layer != layer_end {
                    let dep_tex: Ref<Texture> = ptr::read(&raw const (*layer).texture);
                    if dep_tex.view::<Mut>().file_textures().count > 0 {
                        let dst: *mut OrderedTexture = uc.tmp_stack_view().push(1);
                        ufbxi_check!(uc, !dst.is_null(), "dst");
                        (*dst).texture = dep_tex.ptr();
                        (*dst).order = num_deps;
                        num_deps += 1;
                    }
                    layer = layer.add(1);
                ...
                if !shader.is_null() {
                    ...
                    while input != input_end {
                        if let Some(dep_tex) = ptr::read(&raw const (*input).texture) {
                            if dep_tex.view::<Mut>().file_textures().count > 0 {
                                let dst: *mut OrderedTexture = uc.tmp_stack_view().push(1);
                                ufbxi_check!(uc, !dst.is_null(), "dst");
                                (*dst).texture = dep_tex.ptr();
            ...
            unsafe {
                if (*texture).type_ == TextureType::File {
                    ...
                    *((*texture).file_textures.data as *mut *mut Texture).add(0) = texture;
                    ...
                    if ptr::read(&raw const (*texture).shader).is_none() {
                        *states.add((*texture).element.typed_id as usize) =
                            FILE_TEXTURE_FETCH_FINISHED as u8;
                        continue;
                    }
                ...
                *states.add((*texture).element.typed_id as usize) =
                ...
                if !shader.is_null() {
                    ...
                    let input_end: *mut ShaderTextureInput = add_ptr(input, (*shader).inputs.count);
                    while input != input_end {
                        if ptr::read(&raw const (*input).texture).is_some() {
                            ufbxi_check!(
                                uc,
                                !uc.tmp_stack_view().push_copy_raw::<*mut Texture>(1,
...
pub(crate) fn modify_geometry<'a>(uc: &'a Context) -> Result<(), Fail> {
    ...
    if uc.opts_view().geometry_transform_handling() != GeometryTransformHandling::Preserve {
        ...
        let mut defaults: Option<&PropsView> = None;
        // SAFETY: walks the stored `nodes` element-pointer run of the uc-owned scene
        // (`count` entries); each `PropsView` is minted over that node's own props,
        // arena memory with write-capable provenance, live for the mint's use.
        unsafe {
            // C: `ufbxi_for_ptr_list(ufbx_node, p_node, uc->scene.nodes)`
            let mut p_node: *mut *mut Node = uc.scene_view().nodes_view().data() as *mut *mut Node;
            let p_node_end: *mut *mut Node = add_ptr(p_node, uc.scene_view().nodes_view().count());
            while p_node != p_node_end {
                let node: *mut Node = *p_node;
                if defaults.is_none() {
                    defaults = ptr::read(&raw const (*node).element.props.defaults).map(Ref::view);
                }
...
pub(crate) unsafe fn finalize_scene<'a>(uc: &'a Context) -> Result<(), Fail> {
    ...
    for type_ in 0..ELEMENT_TYPE_COUNT {
        ...
        // C repeats the free of the same, already emptied buffer here;
        // `buf_free` nulls `chunks`, so this second call is the no-op C's is.
        buf_free(uc.tmp_typed_element_offsets_at(type_));
    }
...

    {
        ...
        for mesh_ix in 0..scene_meshes.count() {
            ...
            fetch_dst_elements(
                ...
                ElementNodeWalk::from_mesh(mesh),
                search_node,
                true,
                None,
    ...
    // Ugh.. Patch the textures from meshes for legacy LayerElement-style textures
    {
        ...
        while p_mesh != p_mesh_end {
            ...
            // SAFETY: the MaybeUninit slot is live, aligned, stable for this
            // scope, and write-capable. A Mut View may carry its uninitialized
            // bytes; the fetch writes data then count before any getter below.
            let textures_view: &RefListView<Texture> =
                unsafe { RefListView::<Texture>::from_ptr(textures_storage.as_mut_ptr()) };
            fetch_dst_elements(
                uc,
                element_flags,
                textures_view.as_element_list(),
                ElementNodeWalk::from_mesh(mesh),
                true,
                false,
                None,
...
pub(crate) fn update_pose(pose_view: &PoseView) {
    ...
    while bone != bone_end {
        ...
        if let Some(bone_pose) = get_bone_pose_entry(Some(pose_view), node.parent_view()) {
            ...
            // outlives this loop.
            parent_to_world = parent.node_to_world_ptr();
        }
...
pub(crate) fn update_texture(texture_view: &TextureView) {
    ...
    if let Some(shader) = texture_view.shader_view() {
...
pub(crate) fn update_anim_stack<'a>(scene: &'a SceneView, stack_view: &'a AnimStackView) {
    ...
    // SAFETY: `begin`/`end` are null-checked before the derefs; `scene` and
    // `stack` are storage of the views passed in.
    unsafe {
        if !begin.is_null() && !end.is_null() {
    ...
    let anim: &View<Anim> = stack_view.anim_view();
    anim.set_time_begin(stack_view.time_begin());
    anim.set_time_end(stack_view.time_end());
}
...
pub(crate) fn update_constraint(constraint_view: &ConstraintView) {
    ...
    unsafe {
        ...
        while target != target_end {
            let node: &NodeView = ptr::read(&raw const (*target).node).view();
...
pub(crate) fn update_initial_clusters(scene_view: &SceneView) {
    ...
    for i in 0..skin_clusters.count() {
        ...
        if node_view.is_geometry_transform_helper() {
            ...
            // the node it serves, so its parent link is set.
            node_view = unsafe { node_view.parent_view().unwrap_unchecked() };
        }
...
pub(crate) fn update_adjust_transforms<'a>(uc: &'a Context, scene: &'a SceneView) {
    ...
    unsafe {
        ...
        while p_node != p_node_end {
            ...
            if let Some(parent) = ptr::read(&raw const (*node).parent) {
                ...
                let parent_view: &'a NodeView = parent.view();
                if parent_view.has_root_adjust_transform()
                ...
                if parent_view.is_scale_compensate_parent()
            ...
            if (*node).all_attribs.count == 1 {
                ...
                if has_light_transform {
                    if let Some(light) = ptr::read(&raw const (*node).light) {
                        (*node).adjust_post_rotation = light_post_rotation;
                        light.view::<Mut>().set_local_direction(light_direction);
                        (*node).has_adjust_transform = true;
                    }
                }
                if has_camera_transform {
                    if let Some(camera) = ptr::read(&raw const (*node).camera) {
                        (*node).adjust_post_rotation = camera_post_rotation;
                        camera
                            .view::<Mut>()
                            .set_projection_axes(uc.opts_view().target_camera_axes());
                        (*node).has_adjust_transform = true;
                    }
                }
            }
...
mod tests {
    ...
    fn element_node_walks_helpers_and_instances() {
        ...
        // Stored Refs retain the scene arena's whole-allocation provenance, so
        // the cursor can keep typed Node states while following helper parents.
        ...
        let mesh_view: &View<Mesh> = mesh.view();
        let expected = mesh_view.element().instances_view().at(0);
        ...
        assert_eq!(mesh_walk.element().as_ptr(), expected.element().as_ptr());
