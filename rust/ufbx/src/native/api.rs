    ...
    while begin < end {
        ...
        match (*prev).interpolation {
            ...
            Interpolation::Linear => {
                ...
                return ((*prev).value as f64 * (1.0 - t) + (*next).value as f64 * t) as Real;
            }
            ...
            Interpolation::Cubic => {
                ...
                let y0: f64 = (*prev).value as f64;
                let y3: f64 = (*next).value as f64;
...
pub(crate) unsafe fn bake_anim(
    ...
    // C: `ufbxi_free(&(*bc.get()).ator_tmp, char, bc.tmp_arr(), (*bc.get()).tmp_arr_size);`
    free::<u8>(
...
pub(crate) unsafe fn quat_mul(a: Quat, b: Quat) -> Quat {
    mul_quat(a, b)
}
...
#[inline(never)]
pub(crate) unsafe fn quat_slerp(a: Quat, mut b: Quat, t: Real) -> Quat {
    ...
    let mut dot: f64 = (a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w) as f64;
    if dot < 0.0 {
        dot = -dot;
        b.x = -b.x;
        b.y = -b.y;
    ...
    let af: f64 = math::sin((1.0 - t as f64) * omega) * rcp_so;
    let bf: f64 = math::sin(t as f64 * omega) * rcp_so;

    // C: `af*a.x + bf*b.x` — `af`/`bf` are double, so the components promote.
    let x: f64 = af * a.x as f64 + bf * b.x as f64;
    let y: f64 = af * a.y as f64 + bf * b.y as f64;
    let z: f64 = af * a.z as f64 + bf * b.z as f64;
    let w: f64 = af * a.w as f64 + bf * b.w as f64;
    let rcp_len: f64 = 1.0 / math::sqrt(x * x + y * y + z * z + w * w);

    // C: `ufbx_quat ret;` — all four fields are written below.
    let mut ret: Quat = core::mem::zeroed();
...
pub(crate) unsafe fn euler_to_quat(v: Vec3, order: RotationOrder) -> Quat {
    ...
    let vx: f64 = v.x as f64 * (DEG_TO_RAD_DOUBLE * 0.5);
    let vy: f64 = v.y as f64 * (DEG_TO_RAD_DOUBLE * 0.5);
    let vz: f64 = v.z as f64 * (DEG_TO_RAD_DOUBLE * 0.5);
...
pub(crate) fn quat_to_euler(q: Quat, order: RotationOrder) -> Vec3 {
    ...
    let qx: f64 = q.x as f64;
    let qy: f64 = q.y as f64;
    let qz: f64 = q.z as f64;
    let qw: f64 = q.w as f64;
...
pub(crate) unsafe fn matrix_invert(m: *const Matrix) -> Matrix {
    ...
    if math::fabs(det as f64) <= math::EPSILON as f64 {
...
pub(crate) unsafe fn matrix_to_transform(m: *const Matrix) -> Transform {
    ...
    if math::fabs((len - 1.0) as f64) > math::EPSILON as f64 {
        if math::fabs(len as f64) <= math::EPSILON as f64 {
            t.rotation = IDENTITY_QUAT;
        } else {
            t.rotation.x /= len;
            t.rotation.y /= len;
...
pub(crate) unsafe fn catch_get_skin_vertex_matrix(
    ...
    if math::fabs((total_weight - 1.0) as f64) > math::EPSILON as f64 {
        let rcp_weight: Real = if math::fabs(total_weight as f64) > math::EPSILON as f64 {
            1.0 / total_weight
        } else {
            0.0
        };
