...
pub(crate) unsafe fn quat_slerp(a: Quat, mut b: Quat, t: Real) -> Quat {
    let mut dot: f64 = a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w;
    if dot < 0.0 {
        dot = -dot;
        b.x = -b.x;
        b.y = -b.y;
    ...
    let af: f64 = math::sin((1.0 - t) * omega) * rcp_so;
    let bf: f64 = math::sin(t * omega) * rcp_so;

    let x: f64 = af * a.x + bf * b.x;
    let y: f64 = af * a.y + bf * b.y;
    let z: f64 = af * a.z + bf * b.z;
    let w: f64 = af * a.w + bf * b.w;
...
pub(crate) unsafe fn catch_get_skin_vertex_matrix(
    ...
    if math::fabs(total_weight - 1.0) > math::EPSILON {
        let rcp_weight: Real = if math::fabs(total_weight) > math::EPSILON {
            1.0 / total_weight
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_quat_math_family() {
        unsafe {
            ...
            assert!((quat_dot(n, n) - 1.0).abs() < 1e-12);
            assert!(quat_close(
                quat_normalize(Quat {
                    x: 0.0,
                    y: 0.0,
    ...
    fn test_quat_to_euler_all_orders() {
        unsafe {
            ...
            for order in orders {
                ...
                assert!(
                    (back.x - euler.x).abs() < 1e-9
                        && (back.y - euler.y).abs() < 1e-9
                        && (back.z - euler.z).abs() < 1e-9,
                    "order {:?}: {:?}",
                    order as u32,
