...
| `find_baked_node`, `find_baked_element`, `find_baked_node_by_typed_id`, `find_baked_element_by_element_id` | `(&mut BakedAnim, &'a mut Node) -> Option<&'a BakedNode>` and `(&mut BakedAnim, &'a mut Element) -> Option<&'a BakedElement>`: result bound to the key; `(&mut BakedAnim, u32) -> Option<&'a T>`: result bound to nothing | `(&'a BakedAnim, &Node) -> Option<&'a BakedNode>`, `(&'a BakedAnim, &Element) -> Option<&'a BakedElement>`, `(&BakedAnim, u32) -> Option<&T>` (elided: bound to the bake) | Soundness fix plus a usability restore. The results are entries of `bake->nodes` / `bake->elements` (`ufbx_find_baked_node` ufbx.c:31320 → `ufbx_find_baked_node_by_typed_id`, a lower-bound search), so binding them to the key (or to `'static`) allows use after the bake is freed. The `&mut` parameters were bindgen artifacts of C's non-const pointers; nothing is written, and the port mints read-only views. Taking `&BakedAnim` / `&Node` / `&Element` lets every 0.11.2 caller keep compiling (`&mut T` coerces to `&T` at the call site) while several results may be held at once, which a `&'a mut BakedAnim` binding would forbid. Unchanged in ufbx-rust 0.11.3. Generator `override_functions`, `NOTE(ufbx-rs-native)` there; `tests/miri.rs` `baked_finders_shared_borrow` pins the coercion and the multi-result hold. |
